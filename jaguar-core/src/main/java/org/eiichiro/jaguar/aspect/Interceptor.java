 ...
 * implementation to intercept the method invocation and invoke aspect 
 * components.
 * {@code Interceptor} is constructed by {@link Assembler} internally and 
...
public class Interceptor implements MethodInterceptor {
	...
	/**
	 * Constructs a new {@code Interceptor} with the specified {@link Container} 
	 * and intercepted component {@link Descriptor}.
	 * 
	 * @param container
	 * @param descriptor
	 */
	public Interceptor(Container container, Descriptor<?> descriptor) {
					...
					for (Method method : advices.get(descriptor)) {
						...
						try {
							if (method.getParameterTypes().length == 0) {
								method.invoke(aspects.get(descriptor));
							} else {
								method.invoke(aspects.get(descriptor), args);
							}
